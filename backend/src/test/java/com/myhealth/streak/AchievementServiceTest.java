...
class AchievementServiceTest {
    ...
    void reconcile_awardsEveryNewlyEarnedBadge_andBuildsFullWall() {
        ...
        StreakMetrics metrics = new StreakMetrics(7, 100, 10, 1, 0, 0, 0, 0);
    ...
    void reconcile_isIdempotent_doesNotReAwardExisting() {
        ...
        StreakMetrics metrics = new StreakMetrics(3, 0, 0, 0, 0, 0, 0, 0); // only STREAK_3 earned, already owned
    ...
    void reconcile_reportsProgress_forLockedBadge() {
        StreakMetrics metrics = new StreakMetrics(0, 30, 0, 0, 0, 0, 0, 0); // 30/50 meals
    ...
    void reconcile_awardsPhotoAndWeeklyGoalBadges() {
        ...
        StreakMetrics metrics = new StreakMetrics(0, 0, 0, 0, 25, 4, 0, 0);
    ...
    void reconcile_locksWeeklyGoalBadge_belowThreshold() {
        StreakMetrics metrics = new StreakMetrics(0, 0, 0, 0, 2, 0, 0, 0); // 2 photos, no weekly hits
    ...
    @Test
    void reconcile_awardsHabitStreakBadge_atSevenDays() {
        // A 7-day single-habit streak earns HABIT_7 but not yet HABIT_30.
        StreakMetrics metrics = new StreakMetrics(0, 0, 0, 0, 0, 0, 0, 7);

        AchievementService.ReconcileResult res = service.reconcile(user, metrics);

        assertThat(res.newlyUnlocked()).contains("HABIT_7");
        assertThat(view(res.views(), "HABIT_7").unlocked()).isTrue();
        assertThat(view(res.views(), "HABIT_30").unlocked()).isFalse();
        assertThat(view(res.views(), "HABIT_30").progress()).isEqualTo(7);
    }

    @Test
    void reconcile_awardsScheduleCompletionBadges() {
        StreakMetrics metrics = new StreakMetrics(0, 0, 0, 0, 0, 0, 3, 0);

        AchievementService.ReconcileResult res = service.reconcile(user, metrics);

        assertThat(res.newlyUnlocked()).contains("SCHEDULE_3").doesNotContain("SCHEDULE_12");
        assertThat(view(res.views(), "SCHEDULE_3").unlocked()).isTrue();
        assertThat(view(res.views(), "SCHEDULE_12").progress()).isEqualTo(3);
    }
    ...
    void reconcile_concurrentInsertRace_marksUnlockedButNotNewly() {
        ...
        StreakMetrics metrics = new StreakMetrics(0, 0, 0, 1, 0, 0, 0, 0); // earns FIRST_WEIGHT
