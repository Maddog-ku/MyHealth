...
| `SERVICE_UNAVAILABLE` | 503 | **資料庫暫時連不上**（連線被拒、連線池取不到連線）| 基礎設施問題，**非程式 bug**。見 [§3](#3-崩潰排查症狀--根因--解法)。前端對 5xx 會自動重試 2 次 |
| `INTERNAL_ERROR` | 500 / 503 | 未被預期攔截的例外（500）；或 **Redis 限流後端不可用且 `fail-open=false`**（503）| **看 `.dev-logs/backend.log` 的 stack trace**。這是唯一需要真正 debug 程式的代碼 |

### 已宣告但目前不會出現的代碼

| `error` | 狀態 | 說明 |
|---------|------|------|
| `TOKEN_EXPIRED` | — | enum 中存在，但目前 JWT 進入點（`SecurityConfig`）對所有 token 問題一律回 `UNAUTHORIZED` + `"token invalid or expired"`，不細分過期 |
| `AI_UNAVAILABLE` | — | enum 中存在，但本地 AI provider 失敗時會**回退到內建模板**而非拋錯，因此一般不會看到此代碼 |

> ⚠️ **已知不一致**：Redis 限流後端不可用時，`RedisBucket4jRateLimitStore` 回的是 `503` 但 `error` 是 `INTERNAL_ERROR`（理應是 `SERVICE_UNAVAILABLE`）。排查時以 HTTP 503 + log 訊息「rate limit backend unavailable」為準。
...
curl -s -o /dev/null -w '%{http_code}\n' \
  http://localhost:8080/api/v1/me                    # 401=後端正常；503=DB 掛；000=後端沒起
```
